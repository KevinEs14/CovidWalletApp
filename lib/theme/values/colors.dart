import 'package:flutter/material.dart';

const Color colorPrimary = Color(0xff8DA7FA);
const Color colorOnPrimary=Color(0xffFFFFFF);

const Color colorSecondary= Color(0xffE99F30);
const Color colorOnSecondary = Color(0xFFFFFFFF);

const Color colorBackground=Color(0xffF1F2F7);
const Color colorOnBackground=Color(0xffFFFFFF);

const Color colorError=Color(0xffB14D4D);
const Color colorOnError = Color(0xFFFFFFFF);


const Color colorSurface = Color(0xFFFFFFFF);
const Color colorOnSurface = Color(0xFF444444);

const List<Color> colorCards=[Color(0xffEB9522),Color(0xffB14D4D),Color(0xff5E447D),Color(0xff39609B),Color(0xff209FA8)];

const Color colorTextPrimary=Color(0xff707070);
const Color colorTextSecondary=Color(0xffFFFFFF);
